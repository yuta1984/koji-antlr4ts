...
describe('ast', function () {
  ...
  it('has an array of parent', function () {
    ...
    expect(ast.parens[1]).deep.equals({
      start: 5,
      stop: 12,
    });
    expect(ast.parens[0]).deep.equals({
      start: 0,
      stop: 13,
    });
  });
  ...
  describe('on handling syntactic sugars', () => {
    ...
    it('allows furigana for ？ □　■ 【】', function () {
      ...
      texts.forEach((text) => {
        ...
        const furigana = result.ast.body[0] as KojiASTNode;
        expect(result.errors).to.be.empty;
        expect(furigana.children[1][0]).to.equal(
          'ふりがな'
        );
      });
    });
    ...
    it('recognizes okurigana and kaeriten', () => {
      ...
      expect(children[1]).to.have.property(
        'name',
        '送り仮名'
      );
      expect(children[2]).to.have.property(
        'name',
        '返り点'
      );
      expect(children[3]).equal('吹');
      expect(children[4]).to.have.property(
        'name',
        '送り仮名'
      );
    });
    ...
    it('recognizes all the kaeriten', () => {
      const kaeriten =
        'レ一二三四五六七八九十上中下甲乙丙丁天地人'.split(
          ''
        );
      kaeriten.forEach((k) => {
        const src = `犬＿${k}`;
        const ast = parse(src).ast;
        expect(ast.body[1]).to.have.property(
          'name',
          '返り点'
        );
      });
