...
だゝる神社佛閣（しんじやふつかく）にたり＿一￣ヲ ては十に九ツ恙（つつが）なきは奇（あや）しくもいと尊（たふと）し仰（あふ）かざらめや
...
称（とな）へて其徴（そのしるし）は天下（てんか）安穩（あんおん）五穀（ごこく）豊饒（ぶねう）天子（てんし）吉大臣（きつだいじん）福（く）を受（うけ）万民（はんみん）安穩（あんをん）也（なり）としるせり火神動（くわじんゆり）
龍神動（りうじんゆり）金翅鳥動（こんしてうゆり）と震動（ぢしん）に四種（ししゆ）ある事大智度論（たいちどろん）に出て帝釈動（たいしやくゆり）の外（ほか）は皆（みな）凶兆（きようてう）なり大
...
describe('case1', () => {
	it('should be parsed without errors', function() {
		const result = parse(case1);
		expect(result.errors).lengthOf(0);
		console.log(result.ast.inlines);
	});
});
