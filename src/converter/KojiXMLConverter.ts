...
export class KojiXMLConverter implements KojiConverter {
  inlineConversionRuleMap: {
    [str: string]: ConversionRule;
  } = {};
  blockConversionRuleMap: {
    [str: string]: ConversionRule;
  } = {};
  ...
  constructor(options?: ConversionOptions) {
    ...
    if (options && options.rules) {
      options.rules.forEach((rule) => {
        ...
        if (rule.type === 'inline') {
          this.inlineConversionRuleMap[rule.elemName] =
            rule;
  ...
  convert(ast: KojiDocumentNode) {
    ...
    return xmlFormat(`<text>${children.join('')}</text>`);
  }
  ...
  convertInline(node: KojiASTNode): string {
    if (this.inlineConversionRuleMap[node.name]) {
      const rule: ConversionRule =
        this.inlineConversionRuleMap[node.name];
      return Mustache.render(
        rule.xmlTemplate,
        this.buildMustacheVariables(node)
      );
    } else {
      let idStr = '',
        classesStr = '';
      ...
      if (node.classes)
        classesStr = `subtype='${node.classes.join(' ')}'`;
  ...
  convertBlock(node: KojiBlockNode): string {
    if (this.blockConversionRuleMap[node.name]) {
      const rule: ConversionRule =
        this.blockConversionRuleMap[node.name];
      return Mustache.render(
        rule.xmlTemplate,
        this.buildMustacheVariables(node)
      );
      ...
      if (node.classes)
        classesStr = `type='${
          node.name
        }' subtype='block ${node.classes.join(' ')}'`;
      const children = this.convertChildren(
        node.children
      ).join('');
  ...
  private buildMustacheVariables(node: KojiASTNode) {
    ...
    vars['$classes'] = node.classes
      ? `${node.classes.join(' ')}`
      : '';
    vars['$xmlId'] = node.id ? `xml:id="${node.id}"` : '';
    vars['$xmlSubtypes'] = node.id
      ? `subtype="${node.classes.join(' ')}"`
      : '';
    return vars;
  }
  ...
  private isInlineNode(
    block: any
  ): block is KojiInlineNode {
