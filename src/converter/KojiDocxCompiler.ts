...
export async function toBuffer(ast: KojiDocumentNode): Promise<Buffer> {
...
export async function toBase64String(ast: KojiDocumentNode): Promise<string> {
    const zip = compile(ast);
    const zipData = await zip.generateAsync({
        type: "base64",
        mimeType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
...
export async function toBlob(ast: KojiDocumentNode): Promise<Blob> {
    const zip = compile(ast);
    const zipData = await zip.generateAsync({
        type: "blob",
        mimeType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
