...
import {
  ...
  toBase64String,
  toBuffer,
} from './converter/KojiDocxCompiler';
...
export function convertToDocxDocumentXml(
  ast: KojiDocumentNode,
  options?: ConversionOptions
): string {
...
type ObjectType<T> = T extends 'blob'
  ? Blob
  : T extends 'base64'
  ? string
  : T extends 'buffer'
  ? Buffer
  : never;
export function convertToDocx<T extends TypeName>(
  ast: KojiDocumentNode,
  type: T
...
export const conversionRules: {
  [elemName: string]: ConversionRule;
} = {};
inlineConversionRules.forEach(
  (r) => (conversionRules[r.elemName] = r)
);
blockConversionRules.forEach(
  (r) => (conversionRules[r.elemName] = r)
);
